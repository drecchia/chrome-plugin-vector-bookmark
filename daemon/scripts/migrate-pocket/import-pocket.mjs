#!/usr/bin/env node
// =============================================================================
// import-pocket.mjs — GetPocket (Obsidian .md) → Vector Bookmarks daemon
// =============================================================================
//
// PURPOSE
//   Migrate a folder of GetPocket-exported markdown bookmarks into the local
//   `vbmd` daemon. Per file, the script:
//     1. parses YAML frontmatter      (url, time_added, tags, tags fixed)
...
//
// SOURCE FRONTMATTER (one `*.md` per bookmark)
//   ---
//   url: https://example.com/...
//   time_added: 1668098076          # unix seconds → visitTs * 1000
...
//
// STATE FILE (resumable; default ./.import-pocket.state.json)
...
//     --dir "/mnt/d/obsidian-vaults/.../Get Pocket Raw"        \  # required
...
//     [--state ./.import-pocket.state.json]                      \
//     [--user-agent "..."]                                       \
//     [--dry-run]          [--interactive]                       \
//     [--skip-indexed=false]                                     \
//     [--no-suggest-tags]  [--min-text 100]                      \
//     [--delete-after-ingest]  [--delete-dead]
//
...
//   node import-pocket.mjs --dir "..." --dry-run --limit 5
//
//   # single-shot debugging with full body preview
//   node import-pocket.mjs --dir "..." --interactive --limit 10
//
//   # full migration with cleanup
//   node import-pocket.mjs --dir "..." --concurrency 3 \
//     --delete-after-ingest --delete-dead --min-text 100
//
// REQUIREMENTS
...
const DIR = args.dir || process.env.POCKET_DIR;
...
const STATE_PATH = args.state || path.join(process.cwd(), ".import-pocket.state.json");
...
async function parseFrontmatter(fp) {
  ...
  for (const line of m[1].split(/\r?\n/)) {
    const mm = line.match(/^([A-Za-z][A-Za-z0-9 _-]*):\s*(.*)$/);
    if (!mm) continue;
    fm[mm[1].trim()] = mm[2].trim();
  }
  const url = fm.url || fm.URL || fm.source || fm.link || null;
  const ts = parseInt(fm.time_added || "0", 10);
  const visitTs = ts > 0 ? ts * 1000 : Date.now();
  const tagsFixed = (fm["tags fixed"] || "").split(",").map((s) => s.trim()).filter(Boolean);
  const tagsPipe = (fm.tags || "").split("|").map((s) => s.trim()).filter(Boolean);
  const tags = Array.from(new Set(tagsFixed.length ? tagsFixed : tagsPipe));
  return { url, tags, visitTs };
}
