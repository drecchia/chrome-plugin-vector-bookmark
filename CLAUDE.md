...
Vector Bookmark é um sistema de memória semântica de navegação composto por dois artefatos: uma extensão Chrome MV3 (TypeScript) e um daemon nativo Go (`vbmd`). A extensão captura passivamente páginas em que o usuário passa ≥30s, extrai o texto principal e envia ao daemon via HTTP local. O daemon indexa o conteúdo em SQLite com busca híbrida BM25+vetores e expõe uma API REST em `127.0.0.1:7532` (padrão). Toda a stack roda exclusivamente na máquina do usuário — sem servidores externos em v0.1.
...
│   ├── cmd/vbmd/main.go     Entrypoint: server mode + Windows env file loader
│   ├── internal/nm/         Cross-platform data dir (DataDir())
│   ├── internal/server/     HTTP router (chi), handlers, CORS middleware
...
│   └── install/             install.sh, vbmd.service, install.ps1
├── proto/types.ts           Tipos TypeScript compartilhados da API HTTP/WS
├── docs/
...
- **Nunca** commitar `daemon/bin/`, `extension/dist/`, `extension/node_modules/`, `*.db` (protegidos pelo `.gitignore`)
